...
async function main() {
  ...
  const unit: Array<[string, boolean]> = [
    ...
    ["Đại vận ở cung Mệnh phi Hóa Lộc nhập cung Nô bộc và Hóa Kị nhập cung Điền trạch", false],
    ["Cung Điền trạch phi hóa kỵ nhập ĐV. Điền trạch", false],
      ...
      for (const hour of [0, 3, 6, 9]) {
        for (const gender of ["male", "female"] as const) {
          ...
          for (const name of PALACES) {
            ...
            const results = queryPalaceKnowledge({ chart, palace: palace as any, starsInPalace: [], branch: palace.earthlyBranch, limit: 100000 });
            ...
            for (const match of results) {
              ...
              const result = checkConditionIndependently(chart, condition);
  ...
  console.log(`Charts: ${charts}, matches: ${matches}, kiểm độc lập: ${checked}, chưa kiểm được: ${unchecked}`);
